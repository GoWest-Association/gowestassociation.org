...
if ( isset( $_REQUEST['sfgen_manual'] ) ) {
    ...
    salesforce_visionboard_generate();
}


// our function will be executed by wpcron
function salesforce_visionboard_generate() {
    ...
    $visionboard_url = $current_host . '/cuvisionboard/';

    // get the current version of that host
    $file = file_get_contents( $visionboard_url );
    ...
    // replace the title
    $header_final = str_replace( '>GoWest Association<', '>VisionBoard<', $header_final );

    // output the final header and footer to their respective files in the uploads folder.
    file_put_contents( $path_to_uploads . 'salesforce/header-vision.php', $header_final );
    file_put_contents( $path_to_uploads . 'salesforce/footer-vision.php', $footer_final );
}


// register the cronjob
add_action( 'salesforce_visionboard_generate', 'salesforce_visionboard_generate' );
if ( ! wp_next_scheduled( 'salesforce_visionboard_generate' ) ) {
    wp_schedule_event( time(), 'hourly', 'salesforce_visionboard_generate' );
}
